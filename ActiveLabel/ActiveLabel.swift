...
@IBDesignable open class ActiveLabel: UILabel {
    ...
    open var customBaselineOffset: [ActiveType : Double] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        for (type, elements) in activeElements {
            ...
            case .custom:
                attributes[NSAttributedStringKey.font] = customFont[type] ?? font!
                attributes[NSAttributedStringKey.baselineOffset] = customBaselineOffset[type] ?? 0.0
            ...
            case .custom: attributes[NSAttributedStringKey.font] = customFont[type] ?? font!
            }
    ...
    fileprivate func updateAttributesWhenSelected(_ isSelected: Bool) {
        ...
        if let highlightFont = hightlightFont, customFont[type] == nil {
            attributes[NSAttributedStringKey.font] = highlightFont
        } else if let font = customFont[type] {
            attributes[NSAttributedStringKey.font] = font
        }
